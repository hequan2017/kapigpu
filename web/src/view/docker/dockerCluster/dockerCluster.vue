...
<template>
  ...
  <el-input v-model="searchInfo.clusterName" placeholder="搜索条件" />
</el-form-item>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="docker_DockerCluster" />
            <ExportExcel  template-id="docker_DockerCluster" filterDeleted/>
            <ImportExcel  template-id="docker_DockerCluster" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="集群名字" prop="clusterName" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateDockerClusterFunc(scope.row)">编辑</el-button>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="detailShow" :show-close="true" :before-close="closeDetailShow" title="查看">
            <el-descriptions :column="1" border>
                    <el-descriptions-item label="集群名字">
...
const getTableData = async() => {
  const table = await getDockerClusterList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
// 删除行
const deleteDockerClusterFunc = async (row) => {
    const res = await deleteDockerCluster({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
